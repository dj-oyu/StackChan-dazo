...
static void IncrementAesCtrNonce(uint8_t nonce_counter[16]) {
    for (int i = 15; i >= 0; --i) {
...
static int AesCryptCtr(esp_aes_context* ctx, size_t length, size_t* nc_off, uint8_t nonce_counter[16],
                       uint8_t stream_block[16], const uint8_t* input, uint8_t* output) {
    size_t offset = *nc_off;
    for (size_t i = 0; i < length; ++i) {
        if (offset == 0) {
            int ret = esp_aes_crypt_ecb(ctx, ESP_AES_ENCRYPT, nonce_counter, stream_block);
            if (ret != 0) {
                return ret;
            }
            IncrementAesCtrNonce(nonce_counter);
        }
...
bool MqttProtocol::SendAudio(std::unique_ptr<AudioStreamPacket> packet) {
    ...
    *(uint32_t*)&nonce[12] = htonl(++local_sequence_);
    ...
    if (AesCryptCtr(&aes_ctx_, packet->payload.size(), &nc_off, (uint8_t*)nonce.data(), stream_block,
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    udp_->OnMessage([this](const std::string& data) {
        ...
        int ret = AesCryptCtr(&aes_ctx_, decrypted_size, &nc_off, nonce, stream_block, encrypted, (uint8_t*)packet->payload.data());
        if (ret != 0) {
...
void MqttProtocol::ParseServerHello(const cJSON* root) {
    ...
    esp_aes_setkey(&aes_ctx_, (const unsigned char*)decoded_key.data(), 128);
    local_sequence_ = 0;
    remote_sequence_ = 0;
